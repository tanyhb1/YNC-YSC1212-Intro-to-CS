...
let rec calculatestring stk l =
  match l with
  | [] -> rev stk
  | 'X' :: t -> (match stk with
                | [] -> calculatestring stk t
                | _ :: rest_of_stack -> calculatestring rest_of_stack t) 
  | h :: t -> calculatestring (h :: stk) t;;

calculatestring [] ['H';'E';'L';'L';'O'];;

let checkident s1 s2 =
  string_eq (calculatestring [] s1) (calculatestring [] s2);;

checkident ['H'; 'X'; 'X'; 'H'; 'E'; 'L'; 'L'; 'L'; 'X'; 'O';] ['H';'E';'L';'L';'O'];;

(* dictionary data structure *)
(* (key * value) list *)
(* key, value, lookup/find, insert/update, delete, empty *)
