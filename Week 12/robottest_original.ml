...
let follow_line _ =
  ...
  while true do
    ...
    else if ((!counter_right - !counter_left) <= 45)
    then
      (if (!counter_right mod 45 = 0)
       then
         begin
           move_back 2.0;
           counter_right := !counter_right + 90
         end
      else
        begin
...
(* our 'version' of maze *)
(* considering that light.scaled < 45 means black && light.scaled >= 45 means white *)
...
let rec sweep acc deg t =
  ...
  (if ((!light).scaled < 45) then (printf ("exiting turn");)
         ...
         then (if (acc > 0)
           ...
           (if deg
            then
              (if (acc > int_of_float (0.2*.(float_of_int t)))
               then (set_speed (-16) 16; sweep (acc - 1) false (t + 1))
               else (set_speed 16 (-16); sweep (acc + 1) true (t + 1)))
            else
              (if (acc < int_of_float ((-0.2)*.(float_of_int t))) then (set_speed 16 (-16); sweep (acc + 1) true (t + 1))
               else (set_speed (-16) (16); sweep (acc - 1) false (t - 1))))));;
...
let rec turn x acc cw time =
  let get_light = ref (Sensor.get conn `S3) in
  (if ((!get_light).scaled < x) then (printf ("exiting turn");)
   else (if (time > 200)
         then (if (acc > 0)
               then (set_speed (-5) 5; usleep (float_of_int ((abs acc)/40));
                     set_speed (-30) (-30); usleep 0.5; turn x acc cw 0)
               else (set_speed 5 (-5); usleep (float_of_int ((abs acc)/40));
                     set_speed (-30) (-30); usleep 0.5; turn x acc cw 0))
         else
           (if cw
            then
              (if (acc > int_of_float (0.2*.(float_of_int time)))
               then (set_speed (-16) 16; turn x (acc - 1) false (time + 1))
               else (set_speed 16 (-16); turn x (acc + 1) true (time + 1)))
            else
              (if (acc < int_of_float ((-0.2)*.(float_of_int time))) then (set_speed 16 (-16); turn x (acc + 1) true (time + 1))
               else (set_speed (-16) (16); turn x (acc - 1) false (time + 1))))));;

let move_maze x =
  let get_light = ref (Sensor.get conn `S3) in
  while true do
    get_light := (Sensor.get conn `S3);
    if ((!get_light).scaled < x) then
      (set_speed 35 35;)
    else
      (set_speed (-20) (-20);
       usleep 0.2;
